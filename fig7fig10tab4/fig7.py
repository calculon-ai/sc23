...
def main(args):
  ...
  im = ax[0][0].imshow(colors, cmap=my_cmap.reversed(), vmin=vmin,
  ...
  for t in range(len(tps)):
    for p in range(len(pps)):
      if rate[t, p] == 0.0:
        ...
        result = calculon.util.human_format(rate[t, p], precision=0)
        result += '\n'
  ...
  plt.setp(ax[0][0].get_xticklabels(), fontsize=11)
  plt.setp(ax[0][0].get_yticklabels(), fontsize=11)
  ax[0][0].grid(which='minor', color='w', linestyle='-', linewidth=2)
  ax[0][0].tick_params(which='minor', bottom=False, left=False)
  ax[0][0].set_title('(a) Sample rate and HBM usage', fontsize=12)
  ax[0][0].set_ylabel('Infinite', fontsize=12)
  ...
  im = ax[0][1].imshow(colors, cmap=my_cmap, vmin=vmin,
                       vmax=vmax, aspect=0.85, origin='lower')
  ...
  for t in range(len(tps)):
    for p in range(len(pps)):
      if np.isinf(mem2_bw[t, p]):
        ...
        tmp_str = calculon.util.human_format(mem2_bw[t, p], precision=0)
        result  = ''.join(tmp_str.split(' '))
        result += '\n'
        tmp_str = calculon.util.human_format(mem2_cap[t, p], v_type='base2',
                                             precision=0)
        result += ''.join(tmp_str.split(' '))
  ...
  plt.setp(ax[0][1].get_xticklabels(), fontsize=11)
  plt.setp(ax[0][1].get_yticklabels(), fontsize=11)
  ax[0][1].grid(which='minor', color='w', linestyle='-', linewidth=2)
  ax[0][1].tick_params(which='minor', bottom=False, left=False)
  ax[0][1].set_title('(b) Offloading bandwidth and usage', fontsize=12)
  ...
  vmax = np.max(rate)
  colors = rate.copy()
  im = ax[1][0].imshow(colors, cmap=my_cmap.reversed(), vmin=vmin,
  ...
  plt.setp(ax[1][0].get_xticklabels(), fontsize=11)
  plt.setp(ax[1][0].get_yticklabels(), fontsize=11)
  ax[1][0].grid(which='minor', color='w', linestyle='-', linewidth=2)
  ax[1][0].tick_params(which='minor', bottom=False, left=False)
  ax[1][0].set_title('(c) Sample rate and HBM usage', fontsize=12)
  ax[1][0].set_ylabel('512 GiB @ 100 GB/s', fontsize=12)
  ...
  colors = mem2_cap.copy()
  colors[np.isinf(colors)] = vmax
  im = ax[1][1].imshow(colors, cmap=my_cmap, vmin=vmin,
                       vmax=vmax, aspect=0.75, origin='lower')
  ...
  plt.setp(ax[1][1].get_xticklabels(), fontsize=11)
  plt.setp(ax[1][1].get_yticklabels(), fontsize=11)
  ax[1][1].grid(which='minor', color='w', linestyle='-', linewidth=2)
  ax[1][1].tick_params(which='minor', bottom=False, left=False)
  ax[1][1].set_title('(d) Offloading bandwidth and usage', fontsize=12)
